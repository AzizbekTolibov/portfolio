...
### Spatial composition is auto-computed by default, overridable per node
...
one-line content edit in `content/data/projects.json` — no coordinate
anywhere needs touching, because every position downstream of the array
length is recomputed from it, **as long as that node has no override**.

`getPageNodes()` applies `content/data/layout.json` (typed as
`LayoutOverrides` — `pageId → nodeId → partial { x, y, width, height }`) as
its last step, over whatever autoGrid() computed. autoGrid() still runs
unconditionally, so a node with no override still lands in a sensible
default slot; a node with an override is hand-positioned and no longer
auto-placed. The file starts as `{}` (no overrides exist yet) and is meant
to be written by a local content editor, not hand-edited.

Because a node's generation order and its resolved position can now
diverge, the semantic layer's reading order and the FOCUSED-state stepping
order (Tab / arrow keys / Space) are **not** raw generation order — both
...
- `projects.ts` — no longer the data itself: a thin typed loader that reads
  `data/projects.json` and exports it as `Project[]`. Every existing
  `import { projects } from "@/content/projects"` keeps working unchanged.
  A project is:
  ```ts
  {
    slug: string;
...
re-run it after editing photo counts in `data/projects.json`.
